...
namespace Rosto
{
    internal class Methods
    {
        ...
        public static void ItemOrder(string x)
        {
            try
            {
                string connetionString = @"Data Source=DESKTOP-6LC79FJ\LOAI;Initial Catalog=Rosto;Integrated Security=True";
        ...
        public static void SwitchOnWelcomList()
        {
            ...
            {
                Methods.displayMiddleScreen("... Invalid Input ...");
            }
        }
        ...
        {
            using (SqlConnection connection = new SqlConnection(@"Data Source=DESKTOP-6LC79FJ\LOAI;Initial Catalog=Rosto;Integrated Security=True"))
            {
                connection.Open();

                using (SqlCommand command = new SqlCommand($"select CName from Customer where Cid = {Customer.num()}", connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            return reader["CName"].ToString();
                        }
                        return "  ";
                    }
                }
